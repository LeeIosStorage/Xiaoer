...
@implementation XEScrollPage

- (id)initWithFrame:(CGRect)frame
{
    self = [[[NSBundle mainBundle] loadNibNamed:@"XEScrollPage" owner:self options:nil] objectAtIndex:0];
    if (self) {
        //.....
        self.frame = frame;
...
- (void)addSubviewToScrollView:(UIScrollView *)scrollView withURL:(NSString *)url withTag:(NSInteger)tag{
    ...
    button.frame = frame;
    if (tag == -1) {
        button.tag = XE_ADS_BASE_TAG + tag + 1;
    }else if(tag == _dataArray.count){
        button.tag = XE_ADS_BASE_TAG + tag - 1;
    }else {
        button.tag = XE_ADS_BASE_TAG + tag;
    }
...
-(void)handleClickAtAdsButton:(id)sender{
    if (_delegate && [_delegate respondsToSelector:@selector(didTouchPageView:)]) {
        ...
        NSInteger tag = btn.tag - XE_ADS_BASE_TAG;
