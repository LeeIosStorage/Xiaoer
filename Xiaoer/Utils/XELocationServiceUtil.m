...
#import "XELocationServiceUtil.h"
#import <CoreLocation/CoreLocation.h>
#import "XEUIUtils.h"
#import "CLLocation+Sino.h"

@interface XELocationServiceUtil() <CLLocationManagerDelegate>
...
@implementation XELocationServiceUtil

+(XELocationServiceUtil *) shareInstance
{
    static XELocationServiceUtil *_shareInstance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _shareInstance = [[XELocationServiceUtil alloc] init];
    });
...
-(void) getUserCurrentLocation:(LocationBlock) block location:(LocationSucessBlock) locationSucess;
{
    ...
    if (!isDeviceOpen) {
        NSString *errorString = [XEUIUtils documentOfLocationDenied];//@"设备的定位未打开";
        if (block) {
            block(errorString);
        }
...
- (void)locationManager:(CLLocationManager *)manager
    ...
    switch([error code]) {
        case kCLErrorDenied:
            ...
            errorString = [XEUIUtils documentOfLocationDenied];//@"用户把程序定位关了";
            //Do something...
            break;
...
{
    [manager stopUpdatingLocation];
    //    NSLog(@"####WDF  II get location finished, location array = %@",locations);
    if (locations.count) {
        //把标准坐标转成火星坐标返回
        //        NSLog(@"current location = %@", [locations objectAtIndex:0]);
        CLLocation *marsLocation = [[locations objectAtIndex:0] locationMarsFromEarth];
        //        NSLog(@"mars location = %@", marsLocation);
