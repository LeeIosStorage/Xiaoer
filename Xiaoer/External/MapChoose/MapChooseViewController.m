...
//        self.floatingPin.frame = CGRectMake(fframe.origin.x+5, fframe.origin.y+4, 24, 32);
        self.floatingPin.image=[UIImage imageNamed:@"Pin_Ios7"];
    }else{
//        self.floatingPin.frame = fframe;
    }
...
{
    __weak MapChooseViewController *weakSelf = self;
    XEAlertView *alertView = [[XEAlertView alloc] initWithTitle:nil message:errorString cancelButtonTitle:@"确认" cancelBlock:^{
...
//        [LSUIUtils showCommonTip:tipString At:self.mapView];
        [XEProgressHUD lightAlert:tipString];
        NSLog(@"0702 %@\n",tipString);
    }
...
-(UIView *) isNeedAddIndicatorView:(id<MKAnnotation>)annotation
{
    ...
    if (mapAnnotation.isShowIndicator) {
        ...
        if ([XECommonUtils isUpperSDK ]) {
            //解决ios7下默认goalView背景为白色，菊花看不到bug
            indicator.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray ;
            indicator.center = CGPointMake(38, 15);
        }
        //        [draggablePinView addSubview:indicator];
