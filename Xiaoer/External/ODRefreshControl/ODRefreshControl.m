...
{
    //wdf 20131120 设置contentInset后 view还是应该在显示区域的上面一点，要不然下拉刷新时view可能还没出现就开始刷新了
//    self = [super initWithFrame:CGRectMake(0, -(kTotalViewHeight + scrollView.contentInset.top), scrollView.frame.size.width, kTotalViewHeight)];
    ...
    if (self) {
        ...
        _message.frame = CGRectMake(0, 0, 320, 40);
        _message.textAlignment = NSTextAlignmentCenter;
        _message.font = [UIFont systemFontOfSize:13];
        _message.textColor = SKIN_COLOR;
        ...
        //_tintColor = [UIColor colorWithRed:182.0 / 255.0 green:182.0 / 255.0 blue:182.0 / 255.0 alpha:1.0];
        _tintColor = SKIN_COLOR;
...
//            self.frame = CGRectMake(0, -(kTotalViewHeight + self.scrollView.contentInset.top), self.scrollView.frame.size.width, kTotalViewHeight);
            ...
            self.frame = CGRectMake(0, -(MAX(kTotalViewHeight, self.scrollView.contentInset.top)), self.scrollView.frame.size.width, kTotalViewHeight);
        }
        return;
    }
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            _message.center = _activity.center;            
        ...
        BOOL isdragging = self.scrollView.dragging;
        if (!isdragging) {
            _shapeLayer.path = nil;
            _shapeLayer.shadowPath = nil;
            _arrowLayer.path = nil;
...
{    
    if (_refreshing) {
        ...
        _message.text = success ? @"刷新成功" : @"刷新失败";
